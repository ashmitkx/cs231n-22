...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        args = torch.arange(max_len).reshape(-1, 1) * 10000**(-torch.arange(0, embed_dim, 2) / embed_dim)
        pe[:,:,::2] = torch.sin(args)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query_mh = self.query(query).reshape(N, S, H, Hd).transpose(1, 2)  # (N, H, S, Hd)
        key_mh = self.key(key).reshape(N, T, H, Hd).transpose(1, 2)        # (N, H, T, Hd)
        value_mh = self.value(value).reshape(N, T, H, Hd).transpose(1, 2)  # (N, H, T, Hd)
        
        attn_map = torch.matmul(query_mh, key_mh.transpose(2, 3)) * Hd**-0.5 # (N, H, S, T)
        if attn_mask is not None:
            attn_map = attn_map.masked_fill(attn_mask == 0, -torch.inf)
        attn_map = F.softmax(attn_map, dim=3)
        attn_map = self.attn_drop(attn_map)
        
        head_out = torch.matmul(attn_map, value_mh).transpose(1, 2).reshape(N, S, E) # (N, H, S, Hd) -> (N, S, H, Hd) -> (N, S, E)
        output = self.proj(head_out)
