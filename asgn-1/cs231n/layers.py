...
def svm_loss(x, y):
    ...
    correct_class_scores = x[correct_class_idxs].reshape(-1,1) # (N,1)

    margins = x - correct_class_scores + 1 # (N,C)
    ...
    dL_dx = np.zeros_like(margins) # (N,C)
    dL_dx[margins > 0] = 1 # dL_dS[ij] = 1 if margin > 0 else 0, given j != y[i]
    ns_above_margin = np.sum(margins > 0, axis = 1).reshape(-1,1) # (N,1)
    dL_dx[correct_class_idxs] = -ns_above_margin[:,0] # set gradient of correct score positions
